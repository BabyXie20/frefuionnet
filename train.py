...
from dataloaders.dataset_without_b import *
from networks.DWT import VNet
import random
import numpy as np
import torch
import json
...
parser.add_argument('--model', type=str, default='clusterDWT')
...
def validate_loss(model, valloader, criterion, global_iter, writer=None):
    """仅计算验证损失（最小化）"""
    model.eval()
    running_loss, n_samples = 0.0, 0
    with torch.no_grad():
        for batch in valloader:
            vol, seg = batch['image'].cuda(), batch['label'].cuda()
            out = model(vol)
    ...
    if writer is not None:
        writer.add_scalar('Val/Loss', avg_loss, global_iter)
    return avg_loss
...
def train(train_ids, val_ids, test_ids, fold_id=1):
    ...
    criterion = loss_utils.DiceLoss(n_classes=num_classes)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, batch in enumerate(trainloader):
            ...
            if iter_num % 60 == 0:
                plot_losses.append((iter_num, loss.item()))
                logging.info(f'Fold {fold_id}, epoch {epoch_num}, iter {iter_num}: '
                             f'loss {loss.item():.3f}, lr {cur_lr:.6f}')
        ...
        if (epoch_num + 1) % args.val_every_epochs == 0:
            val_loss = validate_loss(model, valloader, criterion, iter_num, writer)
            logging.info(f'Epoch {epoch_num}, iter {iter_num}, Val loss: {val_loss:.6f}')
            ...
            if val_loss < (best_val - args.min_delta):
                ...
                # 保存 best 模型（基于 ValLoss）
                best_path = os.path.join(snapshot_path_tmp, f'{args.model}_best_model.pth')
                torch.save(model.state_dict(), best_path)
                logging.info(f'Improved! Saved BEST model (by ValLoss) to {best_path}')
    ...
    if plot_losses:
        ...
        plt.xlabel('Iteration'); plt.ylabel('Dice Loss'); plt.title('Training Loss Curve')
...
if __name__ == "__main__":
    ...
    # 存分割
    with open(os.path.join(snapshot_path, 'train_ids.txt'), 'w') as f: f.write('\n'.join(train_ids))
    with open(os.path.join(snapshot_path, 'val_ids.txt'),   'w') as f: f.write('\n'.join(val_ids))
    with open(os.path.join(snapshot_path, 'test_ids.txt'),  'w') as f: f.write('\n'.join(test_ids))
    ...
    try:
        dice_mean = metric_mean[0]; dice_std = metric_std[0]
        ...
        dice_dict = {organ: {"mean": round(float(dice_mean[i]), 3), "std": round(float(dice_std[i]), 3)}
                     for i, organ in enumerate(organ_names)}
        with open(dice_json, 'w') as f: json.dump(dice_dict, f, indent=4)
    ...
    logging.info('Final Test (Avg over cases) — '
